...
type WorkTypeHandler struct{ repo domain.WorkTypeRepository }

func NewWorkTypeHandler(repo domain.WorkTypeRepository) *WorkTypeHandler {
	return &WorkTypeHandler{repo: repo}
}
...
func (h *WorkTypeHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	wt, err := h.repo.Create(req.Name, userID)
	if err != nil {
		writeInternalError(w, err)
		return
	}
