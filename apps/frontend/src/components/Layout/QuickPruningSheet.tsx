...
import PruningForm from '../Vineyard/PruningForm'
...
export default function QuickPruningSheet({ open, vineyard, onClose }: Props) {
  const { vineyards } = useVineyardStore()
  const { create } = usePruningStore()
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
        <Typography variant="subtitle1" sx={{ fontWeight: 700, flex: 1 }}>
          Rebschnitt erfassen{target ? ` · ${target.name}` : ''}
        </Typography>
